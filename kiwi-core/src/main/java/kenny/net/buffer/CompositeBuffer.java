...
public class CompositeBuffer extends AbstractBuffer {
    ...
    private Buffer[] components;
    private int[] indices;
    private int lastAccessedComponentId;
    private final boolean gathering;

    public CompositeBuffer(ByteOrder endianness, List<Buffer> buffers, boolean gathering) {
        order = endianness;
     ...
     */
    public List<Buffer> decompose(int index, int length) {
        if (length == 0) {
        ...
        List<Buffer> slice = new ArrayList<>(components.length);

        // The first component
        Buffer first = components[componentId].duplicate();
    ...
    public BufferFactory factory() {
        return HeapBufferFactory.getInstance(order());
    }
    ...
    public Buffer duplicate() {
        Buffer duplicate = new CompositeBuffer(this);
    ...
    public Buffer copy(int index, int length) {
        ...
        Buffer dst = factory().getBuffer(order(), length);
        copyTo(index, length, componentId, dst);
        return dst;
    }

    private void copyTo(int index, int length, int componentId, Buffer dst) {
        int dstIndex = 0;
        int i = componentId;
    ...
    /**
     * Returns the {@link Buffer} portion of this {@link CompositeBuffer} that
     * contains the specified {@code index}. <strong>This is an expert method!</strong>
     *
     * <p>
     * Please note that since a {@link CompositeBuffer} is made up of
     * multiple {@link Buffer}s, this does <em>not</em> return the full buffer.
     * Instead, it only returns a portion of the composite buffer where the
     * index is located
     * </p>
     *
     *
     * @param index The {@code index} to search for and include in the returned {@link Buffer}
     * @return The {@link Buffer} that contains the specified {@code index}
    ...
    public Buffer slice(int index, int length) {
        ...
        List<Buffer> components = decompose(index, length);
        switch (components.size()) {
            ...
            default:
                return new CompositeBuffer(order(), components, gathering);
        }
    }
    ...
    public ByteBuffer[] toByteBuffers(int index, int length) {
        ...
        List<ByteBuffer> buffers = new ArrayList<>(components.length);
     ...
     */
    private void setComponents(List<Buffer> newComponents) {
        ...
        components = new Buffer[newComponents.size()];
        for (int i = 0; i < components.length; i ++) {
            Buffer c = newComponents.get(i);
            if (c.order() != order()) {
